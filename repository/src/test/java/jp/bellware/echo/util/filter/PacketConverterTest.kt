...
class PacketConverterTest {
    ...
    fun convertFromFloat() {
        PacketConverter.convert(listOf(-1f, 0f, 1f).toFloatArray()).toList() shouldBe
            listOf((Short.MIN_VALUE + 1).toShort(), 0, Short.MAX_VALUE)
    }

    @Test
    fun convertFromShort() {
        val converted = PacketConverter.convert(
            listOf(
